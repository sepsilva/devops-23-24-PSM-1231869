...
#### Task 3
- An application is as useful as the data it can have access to; thus we want our web vm to be able to access a more permanent database (Spring deploys an H2 database but its content is volatile) so it can persist data even if the machine running the application stops.
...
- The good part about having persistence is that we can stop both machines using the command `vagrant halt` and then start them up again using `vagrant up` and see that the data is still there.
- Before we added this, after we stopped our application, either by stopping Spring or the web vm, the data would be lost.
...
- Keep in mind that doing this makes it difficult for our version control system to keep track of the state of our project, since Git is not optimized for tracking binary files.
...
- Ideally we could have our application's archive file in a server and tell Vagrant to execute the command `wget` to download it, similar to what happens with the H2 database in the db vm.
...
Also Vagrant is more similar to a container orchestrator, like [Kubernetes](https://kubernetes.io/), that can manage multiple containers, also known as pods (a pod is a group of whales!).  
Vagrant can be seen as a coordinator of virtual machines.
...
The presence of the [DatabaseLoader class](../../CA2/Part2/react-and-spring-data-rest-basic/src/main/java/com/greglturnquist/payroll/DatabaseLoader.java) in our Spring application was the source of a small bug, when our application is running with the H2 database that is not built into Spring.  
Since the DatabaseLoader class is meant to populate the database with some initial data, everytime we restarted our application, the "same data" (internal ID's were different) was being added to the database which caused duplicate entries.
